...
export const CrudPage = ({name, isAdd= true,isDelete=true, isUpdate=true, columns, addColumns,
    ...
    isApprove=false, isReject=false, isMoreDetails=false,approveAPIfn, rejectAPIfn, moreDetailsfn
 }) => {
    ...
    const handleOnMoreDetails = async (row)=>{
      await moreDetailsfn(row._id);
      setReload((prev)=>!prev);
    }
    ...
    let modifiedColumns = [...columns, 
        {
                ...
                renderCell: (params) => (
                  <div>
                    ...
                    {isMoreDetails &&  <Button
                      variant="contained"
                      color="secondary"
                      size="small"
                      style={{ marginRight: 5 }}
                      onClick={() => handleOnMoreDetails(params.row)}
                    >
                      More Info
                    </Button>}
